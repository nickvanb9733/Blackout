...
async function subscribe(token){const ac=new AbortController(),r=await fetch(base+'/api/events?token='+token,{signal:ac.signal});assert.equal(r.status,200);const obj={ac,state:null,host:null,count:0};streams.push(obj);const reader=r.body.getReader(),decoder=new TextDecoder();obj.task=(async()=>{let buffer='';try{while(true){const{value,done}=await reader.read();if(done)break;buffer+=decoder.decode(value,{stream:true});let i;while((i=buffer.indexOf('\n\n'))>=0){const msg=buffer.slice(0,i);buffer=buffer.slice(i+2);if(msg.startsWith('data: ')){const d=JSON.parse(msg.slice(6));obj.state=d.state;obj.host=d.host;obj.count++}}}}catch(e){if(e.name!=='AbortError')throw e}})();await until(()=>obj.state);return obj}
