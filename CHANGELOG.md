...
- Replaced repetitive full-state broadcasts with versioned incremental updates and one shared encoding per crew. Fresh connections, skipped frames and new map epochs receive complete baselines.
- Reduced unchanged input traffic with immediate changes, 200 ms held-input heartbeats and 1 s idle heartbeats. Pending releases flush after acknowledgement; failed requests retry the original sequence to prevent duplicate actions.
- Added display-only interpolation for online player motion, head riding, moving equipment, enemies and projectiles. Respawns and teleports snap; authoritative physics, checkpoints, questions and narration are preserved.
- Respected SSE backpressure: stop buffering new frames for slow connections and resynchronize after draining. Kept compatibility with the previous full-state protocol.
- Guarded online callbacks against applying updates from a session the player has already left.
- Added transport regression coverage and a repeatable local load test with five independent two-player rooms. Outgoing updates fell about 91% in the recorded comparison; see PERFORMANCE.md.
