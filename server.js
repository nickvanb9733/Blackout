...
function encodeRoom(room){room.encoder??=new StateEncoder();return room.encoder.encode(snapshot(room.game),{host:room.host,code:room.code});}
function sendFrame(s,frame){
 if(!s.stream||s.blockedAt)return;
 const payload=s.lastFrame===frame.base?frame.delta:frame.full;
 // write(false) still accepted this frame. Do not enqueue any newer states
 // until drain; a skipped sequence then automatically forces a full baseline.
 const ready=s.stream.write(payload);s.lastFrame=frame.seq;if(!ready)s.blockedAt=Date.now();
}
const server=http.createServer(async(req,res)=>{
 try{
  const url=new URL(req.url,'http://localhost');
  if(url.pathname==='/health'){return json(res,200,{status:'ok',version:'3.4.0',transport:'sse',protocol:2,rooms:rooms.size})}
  if(url.pathname.startsWith('/api/')){
   ...
   if(req.method==='GET'&&url.pathname==='/api/events'){
    ...
    req.socket.setNoDelay(true);res.writeHead(200,{'Content-Type':'text/event-stream','Cache-Control':'no-cache, no-transform','Connection':'keep-alive','X-Accel-Buffering':'no',...headers});res.write(': connected\n\n');s.stream=res;s.protocol=url.searchParams.get('v')==='2'?2:1;s.lastFrame=-1;s.blockedAt=0;
    res.on('drain',()=>{if(s.stream===res)s.blockedAt=0;});
    if(s.protocol===2)sendFrame(s,encodeRoom(s.room));else res.write('data: '+JSON.stringify({host:s.room.host,code:s.room.code,state:snapshot(s.room.game)})+'\n\n');
...
const timer=setInterval(()=>{
 ...
 broadcast++;if(broadcast%2===0){for(const room of rooms.values()){
  let frame,legacy;for(const s of room.members.values()){
   if(!s.stream)continue;if(s.blockedAt){if(Date.now()-s.blockedAt>10000)departure(s);continue;}
   if(s.protocol===2){frame??=encodeRoom(room);sendFrame(s,frame);}
   else{legacy??='data: '+JSON.stringify({host:room.host,code:room.code,state:snapshot(room.game)})+'\n\n';if(!s.stream.write(legacy))s.blockedAt=Date.now();}
  }
 }}
},20);
...
server.listen(PORT,'0.0.0.0',()=>console.log('BLACKOUT v3.4.0 listening on port '+server.address().port));
