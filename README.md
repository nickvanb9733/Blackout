# BLACKOUT: Reclaim the Data Center (v3.4.0)
...
npm run test:load # Ten local clients in five independent co-op rooms
```
...
## What changed in v3.4 — multiplayer performance

- **Smaller updates:** after the initial state, each crew receives only changed values. Names, cosmetics, purchase history and unchanged equipment data are no longer repeated in every update. Both players share one encoded update per room. Gameplay still runs at 60 Hz and network updates at approximately 25 Hz.
- **Fewer input requests:** changes send immediately; an unchanged held key refreshes every 200 ms, and an idle client every second. A release queued behind a request sends as soon as that request finishes. Retries preserve sequence numbers so a lost response cannot repeat a jump or attack.
- **Smoother online motion:** players, moving equipment, enemies and projectiles interpolate between server updates using a short 50 ms display buffer. Server collision and puzzle checks remain authoritative. Respawns, teleports, deaths and map changes snap into place. Cinematics retain their existing shared clock and local voice timing.
- **Slow connections:** an SSE connection under backpressure stops accumulating new updates, then resumes with a fresh baseline if it missed any. Reconnection also starts from a complete state. Older clients remain compatible, but everyone should refresh after updating to receive these improvements.

The local five-room comparison reduced outgoing state traffic from **1,000.8 to 91.9 KB/s (about 91%)**, with ten connected players and zero request errors. This is a short local transport test, not a measurement of your Railway service or a guarantee about Internet latency. See [PERFORMANCE.md](PERFORMANCE.md) for the workload and results.

To update your existing deployment, upload the new project contents to the same GitHub repository, including **server.js**, **public/network.js**, and the rest of **public/**, then redeploy and have everyone refresh the hosted page. Keep the included Dockerfile and one server replica. `/health` reports **3.4.0** after this server version is running. The standalone HTML remains the solo edition; updating that file alone does not update multiplayer. Deploy between sessions because the server restart ends active rooms.
...
| `public/network.js` | Compact state patches, input scheduling/retries and display-only movement interpolation |
...
| `scripts/load-test.mjs` | Starts a disposable local server and compares ten-client co-op traffic |
| `test/` | Dependency-free Node tests |

The server advances gameplay at 60 Hz and streams versioned state updates at approximately 25 Hz using Server-Sent Events. Clients submit authenticated input messages to the same origin. Positions, damage, relay activation, quiz results and progress are decided by the shared server engine. No external multiplayer service is needed. The updated client requests protocol 2; full snapshots establish a baseline on connection and scene changes, followed by smaller patches. Wire numbers are rounded to three decimal places; server physics keeps its original precision.
...
Version 3.4 adds transport round trips across all 40 maps, immutable patch application, reconnect baselines, input heartbeat and retry tests, double-jump queuing, motion interpolation and teleport handling. The real-client tests now exercise compact updates through all nine cutscenes and preserve legacy-client compatibility. The separate ten-client load test checks room isolation and continued updates. All 69 automated tests pass. Offline and hosted-client wiring were also exercised with a DOM/canvas harness. See PERFORMANCE.md for the measured comparison and remaining deployment playtest.
